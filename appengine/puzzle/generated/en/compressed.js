...
*/
var pa={},x,qa,z,ra,sa,ta,ua,va,wa,ya,za,Aa,Ba,Ca,Da;/*
...
5,""),d),g.forEach(c,l));for(h=1;h<arguments.length-1;h++);bg(this,!a.match(this.xd.ei))};f.xd.ji=/(%\d+|\n)/;f.xd.ei=/%1\s*$/;function ei(a,b,c){var d=null;if(1==b||3==b)d=new Zg(a,b);b=new Th(b,c,a,d);a.S.push(b);a.t&&(a.C(),a.Ca());return b}function eg(a,b){for(var c=0,d;d=a.S[c];c++)if(d.name==b)return d;return null}f.jh=function(){return this.L||""};f.Dd=function(a){this.L=a};f.O=function(){return this.ci};f.moveBy=function(a,b){this.ci.translate(a,b)};/*
 ...
 limitations under the License.
*/
function fi(){var a=this;this.p=new Mg(function(){return hi(a)},function(b){var c=hi(a);c&&(r(b.y)&&(a.p.scrollY=-c.Sa*b.y-c.Za),a.p.aa.setAttribute("transform","translate(0,"+(a.p.scrollY+c.Qa)+")"))});this.p.vh=!0;this.gh=[];this.ea=this.A=0;this.ef=[];this.pb=[]}var ii,ji,ki,li,mi,ni;f=fi.prototype;f.Sd=!0;f.ma=8;f.H=function(){this.g=E("g",{},null);this.Ua=E("path",{"class":"blocklyFlyoutBackground"},this.g);this.g.appendChild(this.p.H());return this.g};
...
function Rf(a,b){var c=a.Ld,d=b.nb();if(d&&!/^[\s\xa0]*$/.test(null==d?"":String(d))){var d=d.toLowerCase(),e=c.dc.get(d);e?e.push(b):c.dc.set(d,[b])}}f.removeNode=function(a){var b=this.Ld,c=a.nb();if(c&&!/^[\s\xa0]*$/.test(null==c?"":String(c))){var c=c.toLowerCase(),d=b.dc.get(c);d&&(qb(d,a),d.length&&b.dc.remove(c))}};/*
 ...
 limitations under the License.
*/
function Zi(a,b){this.Ub=M("div","blocklyToolboxDiv");this.Ub.setAttribute("dir",x?"RTL":"LTR");b.appendChild(this.Ub);this.ja=new fi;a.appendChild(this.ja.H());F(this.Ub,"mousedown",this,function(a){Lb(a)||a.target==this.Ub?ug(!1):ug(!0)})}f=Zi.prototype;f.width=0;f.Ve={If:19,Wg:"blocklyTreeRoot",Tg:"blocklyHidden",Ug:"",mf:"blocklyTreeRow",Vg:"blocklyTreeLabel",Xb:"blocklyTreeIcon",kf:"blocklyTreeIconOpen",lf:"blocklyTreeIconNone",Xg:"blocklyTreeSelected"};
...
y=[],y.type=q&&3!=t.type?-1:t.type,y.height=0,b.push(y));y.push(t);t.jc=25;t.ka=q&&1==t.type?20.5:0;if(t.n&&t.n.m){var xa=oi(R(t.n));t.jc=Math.max(t.jc,xa.height);t.ka=Math.max(t.ka,xa.width)}v==g.length-1&&t.jc--;y.height=Math.max(y.height,t.jc);t.$a=0;1==b.length&&(t.$a+=x?-e:e);for(var xa=!1,Bf=0,gb;gb=t.oa[Bf];Bf++){0!=Bf&&(t.$a+=10);var gi=gb.mh();gb.ka=gi.width;gb.Oe=xa&&gb.uc?10:0;t.$a+=gb.ka+gb.Oe;y.height=Math.max(y.height,gi.height);xa=gb.uc}-1!=y.type&&(3==y.type?(k=!0,d=Math.max(d,t.$a)):
...
x=!!a.rtl;ta=e;sa=g;ua=h;I=b;ya=a.maxBlocks||Infinity;a.media?zg=a.media:a.path&&(zg=a.path+"media/");za=c;Aa=l;qa=d;Ba=m;ra=k;va=q;wa=v}
...
"";for(var h=0;h<d.length;h++){var k=document.createElement("div");k.appendChild(document.createTextNode(d[h]));e.appendChild(k)}d=document.getElementById("answers");e=document.getElementById("checkButton");Z.pc(d,e,!0,!0,{width:"25%",left:-1!=Tk.indexOf(Uk)?"5%":"70%",top:"5em"},b?Z.og:Y.pe);Z.lg();if(c.length){ol(c);var l=c[0],m=function(){l.select();Z.yb&&(setTimeout(function(){Tg(l)},150),setTimeout(m,300))};m()}else setTimeout(rl,2E3),T&&Tg(T)}
function rl(){mj("win",.5);for(var a=Fa(J,!1),b=0,c;c=a[b];b++)sl(c,b/a.length*360)}
function sl(a,b){if(Z.yb){var c=J.wb(),d=c.la/2,e=c.V/2,c=a.O(),g=oi(a),h=Math.max(175,Math.min(d,e)-Math.max(g.height,g.width)/2),k=Date.now(),l=b+k/50%360,h=h*(Math.sin(k%5E3/5E3*2*Math.PI)/8+.875),e=h*Math.cos(l*Math.PI/180)+e-g.width/2,g=h*Math.sin(l*Math.PI/180)+d-g.height/2;5>Math.sqrt(Math.pow(e-c.x,2)+Math.pow(g-c.y,2))?(d=e-c.x,c=g-c.y):(c=180*Math.atan2(g-c.y,e-c.x)/Math.PI%360,c=0>360*c?c+360:c,d=Math.round(5*Math.cos(c*Math.PI/180)),c=Math.round(5*Math.sin(c*Math.PI/180)));a.moveBy(d,
c);setTimeout(sl.bind(null,a,b),50)}}function ql(a){var b=document.getElementById("help"),c=document.getElementById("helpButton");Z.pc(b,c,a,!0,{width:"50%",left:"25%",top:"5em"},Z.og);Z.lg()};
