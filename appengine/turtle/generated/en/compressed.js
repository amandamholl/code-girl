...
K=[],K.type=x&&3!=B.type?-1:B.type,K.height=0,b.push(K));K.push(B);B.nd=25;B.Qa=x&&1==B.type?20.5:0;if(B.s&&B.s.w){var jb=Vh(I(B.s));B.nd=Math.max(B.nd,jb.height);B.Qa=Math.max(B.Qa,jb.width)}E==h.length-1&&B.nd--;K.height=Math.max(K.height,B.nd);B.Ob=0;1==b.length&&(B.Ob+=q?-e:e);for(var jb=!1,Wg=0,Yb;Yb=B.Pa[Wg];Wg++){0!=Wg&&(B.Ob+=10);var Sj=Yb.Od();Yb.Qa=Sj.width;Yb.pg=jb&&Yb.xd?10:0;B.Ob+=Yb.Qa+Yb.pg;K.height=Math.max(K.height,Sj.height);jb=Yb.xd}-1!=K.type&&(3==K.type?(m=!0,d=Math.max(d,B.Ob)):
...
function Am(a,b){"string"==typeof a&&(a=document.getElementById(a));a.addEventListener("click",b,!0);a.addEventListener("touchend",b,!0)}function ym(){if(!tm){window.GoogleAnalyticsObject="GoogleAnalyticsFunction";var a=function(){(a.q=a.q||[]).push(arguments)};window.GoogleAnalyticsFunction=a;a.l=1*new Date;var b=document.createElement("script");b.async=1;b.src="//www.google-analytics.com/analytics.js";document.head.appendChild(b);a("create","UA-50448074-1","auto");a("send","pageview")}};var W={Kd:null,g:function(){wm();var a=document.getElementById("linkButton");"BlocklyStorage"in window?(BlocklyStorage.HTTPREQUEST_ERROR=V("Games_httpRequestError"),BlocklyStorage.LINK_ALERT=V("Games_linkAlert"),BlocklyStorage.HASH_ERROR=V("Games_hashError"),BlocklyStorage.XML_ERROR=V("Games_xmlError"),BlocklyStorage.alert=nm.Us,a&&Am(a,BlocklyStorage.link)):a&&(a.style.display="none");document.getElementById("languageMenu").addEventListener("change",W.jr,!0)},Yr:function(a){document.body.innerHTML=
a;a=document.getElementById("blockly");a.style.height=window.innerHeight+"px";rl(a,{media:"media/",readOnly:!0,Hu:-1!=qm.indexOf(rm),scrollbars:!1});a=um("xml","");W.hi("<xml>"+a+"</xml>")},gs:function(a,b){if("BlocklyStorage"in window&&1<window.location.hash.length)BlocklyStorage.retrieveXml(window.location.hash.substring(1));else{var c=null;try{c=window.sessionStorage.nk}catch(d){}c&&delete window.sessionStorage.nk;var e=xm(H),h=b&&xm(H-1);(c=c||e||h||a)&&W.hi(c)}},hi:function(a){W.Kd?W.Kd.setValue(a,
-1):(a=Pg(a),Qg(A,a))},Ls:function(){if(void 0!=typeof ra&&window.localStorage){var a=zm+H;if(W.Kd)var b=W.Kd.getValue();else b=Lg(A),b=Og(b);window.localStorage[a]=b}},Fh:function(){window.location=(tm?"index.html":"./")+"?lang="+rm},jr:function(){if(window.sessionStorage){if(W.Kd)var a=W.Kd.getValue();else a=Lg(A),a=Og(a);window.sessionStorage.nk=a}var a=document.getElementById("languageMenu"),a=encodeURIComponent(a.options[a.selectedIndex].value),b=window.location.search,b=1>=b.length?"?lang="+
a:b.match(/[?&]lang=[^&]*/)?b.replace(/([?&]lang=)[^&]*/,"$1"+a):b.replace(/\?/,"?lang="+a+"&");window.location=window.location.protocol+"//"+window.location.host+window.location.pathname+b},Ah:function(a){if(a){var b=a.match(/^block_id_(\d+)$/);b&&(a=b[1])}Jh(a)},Lu:function(a){return a.replace(/(,\s*)?'block_id_\d+'\)/g,")").trimRight()},Wb:function(a){if("click"==a.type&&"touchend"==W.Wb.Xh&&W.Wb.Wh+2E3>Date.now()||W.Wb.Xh==a.type&&W.Wb.Wh+400>Date.now())return a.preventDefault(),a.stopPropagation(),
!0;W.Wb.Xh=a.type;W.Wb.Wh=Date.now();return!1}};W.Wb.Xh=null;W.Wb.Wh=0;W.Wr=function(){var a=document.createElement("script");a.setAttribute("type","text/javascript");a.setAttribute("src","js-read-only/JS-Interpreter/compiled.js");document.head.appendChild(a)};
...
function gn(){function a(){c.style.top=Math.max(10,d.offsetTop-window.pageYOffset)+"px";c.style.left=b?"10px":"420px";c.style.width=window.innerWidth-440+"px"}document.body.innerHTML=bn();W.g();var b=-1!=qm.indexOf(rm),c=document.getElementById("blockly"),d=document.getElementById("visualization");window.addEventListener("scroll",function(){a();mh(window,"resize")});window.addEventListener("resize",a);a();10>H&&(Xk=3,Wk="#ff0000 #ffcc33 #ffff00 #009900 #3333ff #cc33cc #ffffff #999999 #000000".split(" "));
if(1!=H)var e=document.getElementById("toolbox");rl(document.getElementById("blockly"),{media:"media/",rtl:b,toolbox:e,trashcan:!0});kl("moveForward,moveBackward,turnRight,turnLeft,penUp,penDown,penWidth,penColour,hideTurtle,showTurtle,print,font");document.getElementById("submitButton")&&Am("submitButton",Bm.Mu);e=document.getElementById("slider");Cm=new Om(10,35,130,e);W.gs(1==H?'<xml>  <block type="turtle_move_internal" x="70" y="120">  </block>  <block type="turtle_turn" x="300" y="200">  </block>  <block type="turtle_move_internal" x="220" y="50">  </block>  <block type="turtle_turn" x="600" y="150">  </block>  <block type="turtle_move_internal" x="600" y="330">  </block>  <block type="turtle_turn" x="200" y="500">  </block>  <block type="turtle_move_internal" x="100" y="350">  </block>  <block type="turtle_turn" x="500" y="450">  </block></xml>':
